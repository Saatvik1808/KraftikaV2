          ...
          <Accordion type="single" collapsible className="w-full">
            {faqData.map((item, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border-b border-primary/20 last:border-b-0">
                <AccordionTrigger className="py-4 text-left text-base md:text-lg font-medium text-foreground/90 hover:text-accent-foreground hover:no-underline [&[data-state=open]]:text-primary">
                  {item.question}
                </AccordionTrigger>
